...
export default {
  ...
  methods: {
    ...
    axios.get(`/api/v1/getPlanes/${idC}/`)
    .then((response) => {
      console.log(response.data)
      ...
      Swal.fire({
        title: "Planes agregados error",
        text:
        "Se detecto un error mientras se cargaban los planes: " + error,
        icon: "success",
        confirmButtonText: "Cerrar",
      });
    });
    ...
    elegirPaquete(idPlan){
      ...
      this.generaCambios(99, 'Click', id)
    },
                    ...
                    <table class="table table-sm mb-0" width="100%">
                      ...
                      <tbody v-if="divisa == 2">
                        <tr v-for="servicios in dataServiciosAgregados" v-bind:key="servicios.id">
                          ...
                          <td>
                            <div v-if="servicios.subtotal > 0">
                              <input v-if="servicios.ajusteVenta === true" class="form-control-input" type="checkbox" v-bind:id="servicios.idService + 'check'" value="" @change="mostrarImpuesto(servicios.idService)" checked>
                              <input v-else class="form-control-input" type="checkbox" v-bind:id="servicios.idService + 'check'" value="" @change="mostrarImpuesto(servicios.idService)">
                            </div>
                            <div v-else>
                          ...
                          <td>
                            <select v-if="servicios.ajusteVenta === true" v-bind:id="servicios.idService + 'seleccionImpuesto'" class=" form-control-sm col-md-12 " @change="sumaImpuesto($event, servicios.idService, servicios.subtotal)">
                              <option v-for="opc in options" :key="opc.id" v-bind:value="opc.id" :selected="opc.value == servicios.aumento"> {{ opc.value }}% </option>
                            </select>

                            <!--select v-else hidden v-bind:id="servicios.idService + 'seleccionImpuesto'" class=" form-control-sm col-md-12 " @change="sumaImpuesto($event, servicios.idService, servicios.subtotal)">
                              <option v-for="opc in options" :key="opc.id" v-bind:value="opc.id" :selected="opc.value == servicios.aumento"> {{ opc.value }}% </option>
                            </select-->

                            <input hidden v-if="servicios.ajusteVenta === true" v-bind:value="servicios.sub" v-bind:id="servicios.idService + 'escondeImpuesto'">
                            <input hidden v-else value="0" v-bind:id="servicios.idService + 'escondeImpuesto'">
                          </td>
                          <td>
                            <input v-if="servicios.ajusteVenta === true" :placeholder="parseFloat(servicios.subtotal + servicios.porcAumento)" class="form-control form-control-sm text-center" type="text" value="" v-bind:id="servicios.idService + 'sumaTotal'" readonly>
                            ...
                            <input v-if="servicios.ajusteVenta === true" hidden v-bind:value="servicios.porcAumento" v-bind:id="servicios.idService + 'impuestoOculto'">
                      ...
                      <tbody v-else>
                        <tr v-for="servicios in dataServiciosAgregados" v-bind:key="servicios.id">
                          ...
                          <td>
                            <input v-if="servicios.aumento > 0 && servicios.subtotal > 0 && estatus == 0" class="form-control-input" type="checkbox" v-bind:id="servicios.idService + 'check'" value="" @change="mostrarImpuesto(servicios.idService)" checked>
                            <input v-if="servicios.aumento == 0 && servicios.subtotal > 0 && estatus == 0" class="form-control-input" type="checkbox" v-bind:id="servicios.idService + 'check'" value="" @change="mostrarImpuesto(servicios.idService)">
                          </td>
                          <td>
                            <select v-if="servicios.ajusteVenta === true" v-bind:id="servicios.idService + 'seleccionImpuesto'" class=" form-control-sm col-md-12 " @change="sumaImpuesto($event, servicios.idService, servicios.subtotal)">
                              <option v-for="opc in options" :key="opc.id" v-bind:value="opc.id" :selected="opc.value == servicios.aumento"> {{ opc.value }}% </option>
                    ...
                    <div class="card card-pricing" :class="{ 'ribbon-box': `${price.ribbon}` === 'Más Vendido' }">
                      ...
                      <div class="card-body text-center">
                        ...
                        <ul class="card-pricing-features">
                          <li>{{ price.services }} GB Storage</li>
                        ...
                        <button v-if="price.seleccionado == 1" class="btn mt-4 btn-block btn-secondary" :class="{ 'btn-danger': `${price.ribbon}` === 'Más Vendido' }">
                          <i class="fe-check"></i> Plan Seleccionado
                        </button>
