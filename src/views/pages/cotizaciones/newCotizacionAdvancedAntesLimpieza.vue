...
export default {
  ...
  methods: {
    ...
    selectEstibable() {
      ...
      let altura = parseFloat(this.altoMerc)
      if(altura > 0){
        if (altura > 1.3) {
          this.terestibable = "no"
          if(parseFloat(this.embalajeAlto) > 0) this.altoMerc = parseFloat(this.embalajeAlto);
    ...
    addServicios(idServicio, nombreServicio) {
      ...
      if (nombreServicio == "SEGURO") {
        ...
        "origen": "CDMX",
        "destino": "VERACRUZ",
        "factor_conversion": "350.00",
        "recoleccion_tres_y_media": "1064.00",
        "recoleccion_rabon": "1344.00",
        "recoleccion_torton": "1568.00",
        "flete_nacional": "374.50",
        "entrega_puerto_nissan": "1624.00",
        "entrega_puerto_tres_y_media": "2016.00",
        "entrega_rabon": "2464.00",
        "entrega_torton": "2912.00",
        "usuarioAlta": null,
        "usuarioModifica": null,
        "dateEdita": null,
        "dateCreate": "2023-05-20T00:00:00-06:00",
        "estatus": 1*/

        if (this.definePesoXVol) {

          if(this.totalPesoVolTot > this.totalKilo){
            this.volDef = this.totalPesoVolTot //* this.precioXMetCub;
          }else{
        ...
        let agrega = {
          ...
          aumento: 0,
          porcAumento: 0,
          totalServicio: subtotal + porcIva + porcZPeligrosa + porcNComercial + porcSobrepeso + porcSusceptible + porcVolMerc + this.volDef + this.pesoDef,
        };
        let indice = this.confirmarServices.map((busqueda) => busqueda.idService).indexOf(idServicio);
        this.confirmarServices.splice(indice, 1);
      ...
      if(this.termodalidad == 'LTL'){
        const index = this.units.findIndex((elemento) => elemento.idPaq === id);
        if (index !== -1) {
        ...
        if(this.tipoUnidad == 0){
          for (let j = 0; j < this.agregarMercancias.length; j++) {
            ...
            if(canTotMerc <= canTotUnit && canTotMercTot <= canTotUnit){
              console.log(canTotMerc, '<=', canTotUnit, '&&', canTotMercTot, '<=', canTotUnit, 'UNO')
              if(volTotMerc <= volTotUnit && volTotMercTot <= volTotUnit){
                console.log(volTotMerc, '<=', volTotUnit, '&&', volTotMercTot, '<=', volTotUnit, 'DOS')
                if(pesTotMerc <= pesTotUnit && pesTotMercTot <= pesTotUnit){
                  console.log(pesTotMerc, '<=', pesTotUnit, '&&', pesTotMercTot, '<=', pesTotUnit, 'TRES')
                  if(lonTotMerc <= lonTotUnit && lonTotMercTot <= lonTotUnit){
                    console.log(lonTotMerc, '<=', lonTotUnit, '&&', lonTotMercTot, '<=', lonTotUnit, 'CUATRO')
                    if(widTotMerc <= widTotUnit && widTotMercTot <= widTotUnit){
                      console.log(widTotMerc, '<=', widTotUnit, '&&', widTotMercTot, '<=', widTotUnit, 'CINCO')
                      if(higTotMerc <= higTotUnit && higTotMercTot <= higTotUnit){
                        console.log(higTotMerc, '<=', higTotUnit, '&&', higTotMercTot, '<=', higTotUnit, 'SEIS')
                        this.tipoUnidad = parseInt(this.units[i].id);
                        this.nomUnidadModalidad = this.units[i].name;
                        this.nombreTipoUnidad = this.units[i].name;

                        this.embalajeAlto = higTotMerc;
                        this.embalajeAncho = widTotMerc;
                        this.embalajeLargo = lonTotMerc;
    ...
    agregaPaquetes() {
      ...
      if (pesoT > this.unidadPesoBruto) {
        ...
        if(estibable == 'si'){
          ...
          const numero = 123.45;
          const longitudAntesDelPunto = this.redondearCifra(numero);
          console.log('Longitud antes del punto decimal:', longitudAntesDelPunto);
        }
    ...
    validaDatosMercancias(id) {
      ...
      if (this.agregarMercancias.length > 0) {
        ...
        if (totalKilos > this.unidadPesoBruto) {
          ...
          const index = this.agregarMercancias.findIndex((elemento) => elemento.idPaq === id);
          if (index !== -1) {
            this.agregarMercancias.splice(index, 1);
          }
          return false;
    ...
    agregaPaquetesLTL() {
      ...
      for (let i = 0; i < this.agregarMercancias.length; i++) {
        totalKilos = totalKilos + parseFloat(this.agregarMercancias[i].pesoTotal);
        //console.log(totalKilos, 'totalKilos')
        totalPesoVolTot = totalPesoVolTot + parseFloat(this.agregarMercancias[i].pesoVolumetricoTotal);
        //console.log(totalPesoVolTot, 'totalPesoVolTot') 
      }
    ...
    redondearCifra(numero) {
      const numeroString = numero.toString();
      const indiceDelPunto = numeroString.indexOf('.');
      
      if (indiceDelPunto !== -1) {
        return indiceDelPunto;
      } else {
    ...
    getTarifario(){
      ...
      let origen = this.arrayOrigen[0].estadoo
      let destino = this.arrayDestino[0].estadod
      ...
      axios({
        ...
        this.datosTarifario = response.data
      }).catch((error) => {
        console.log(error);
      });
    },
            ...
            <div class="_2ka8NZmhs89Mkk9ABZSXJM">
              <div class="_1n4EOSyAk1BfVWnF_adPB9">
                ...
                <div class="_23ovY8N9sx2ExP0NvytcpI">
                  ...
                  <div class="_12VTCAtCmgnF7JdGljsEap">
                    <div class="_3vCdC7UlpoMI8zyS9070Tm">
                      <div class="_2OKKnWa2I26FDFjQGrMlTi" style="color: #00786c">
                ...
                <div class="_3QlnruSL5VqOlp6mJEr4-Z">
                  <!--button class="zTDkSCFjS5VtNrkEzKtJ5" style="background-color: #00786c; box-shadow: rgba(4, 114, 235, 0.5) 0px 0px 3px;"></button-->
              ...
              <div class="row _2ka8NZmhs89Mkk9ABZSXJM" id="formOperaciones">
                        ...
                        <div class="col-md-12">
                          <div class="form-group">
                            <label for="descripcion" class="form-label">
                              <b style="color: red">*</b>Ingresa Volumen Total <label style="font-size:20px;" v-if="nomUnidadModalidad != ''"> - <b-badge style="background-color:#00786c;"> {{ nomUnidadModalidad }}</b-badge></label>
                            </label>
                            <div class="input-group">
                          ...
                          <table class="table">
                            <thead class="text-white" style="background-color: rgb(0, 120, 108)">
                              <tr>
                                ...
                                <th scope="col">Volumen Total(m3)</th>
                            ...
                            <tbody style="border: 1px solid #edeff1; border-collapse: collapse;">
                              ...
                              <tr>
                                ...
                                <td style="border: 1px solid #edeff1; border-collapse: collapse;">
                                  <b-form-select v-if="termodalidad != 'LTL'" id="embalaje" class="rounded custom-select-sm" v-b-tooltip.hover="{ variant: 'success' }" v-model.trim="terembalaje" @change="selectEmbalaje($event)">
                                    <option v-for="emb in embalajes" :key="emb.idEmbalaje" v-bind:value="emb.idEmbalaje">
                                      {{ emb.nombre.toUpperCase() }}
                                    </option>
                                  </b-form-select>
                                  <b-form-select v-else id="embalaje" class="rounded custom-select-sm" v-b-tooltip.hover="{ variant: 'success' }" v-model.trim="terembalaje" @change="selectEmbalajeLTL($event)">
                                    <option v-for="emb in embalajes" :key="emb.idEmbalaje" v-bind:value="emb.idEmbalaje">
                                      {{ emb.nombre.toUpperCase() }}
                                    </option>
                                  </b-form-select>
                                ...
                                <td style="border: 1px solid #edeff1; border-collapse: collapse;">
                                  <b-form-select :disabled="altoMerc > 1.3" id="estibable" class="form-control custom-select-sm" v-b-tooltip.hover="{ variant: 'success' }" v-model.trim="terestibable" @change="selectEstibable">
                                ...
                                <td style="border: 1px solid #edeff1; border-collapse: collapse;">
                                  ...
                                  <input v-else class="form-control form-control-sm" v-model="altoMerc" type="number" @input="validaAlto()" />
                                </td>
                                <td style="border: 1px solid #edeff1; border-collapse: collapse;">
                ...
                <b-modal id="valorDeclaradoMercancia" no-close-on-esc no-close-on-backdrop hide-footer centered hide-close>
                  ...
                  <div class="form-group text-center">
                    ...
                    <b-button class="width-md ml-1" style="background-color: #00786c" @click="addServicios(2, 'SEGURO')"><b><i class="fe-check"></i> Guardar</b></b-button>
                  </div>
                </b-modal>
                        ...
                        <b-card-body class="py-2">
                          <div class="row" style="border-bottom: 2px solid #f5f4f4; margin-top: -12px;">
                            ...
                            <div class="col-md-4 text-right mt-2 mb-2">
                              <b-button pill style="background-color: #00786c" :disabled="isActive" v-b-modal.resumen-modal title="Resumen General de Cotizacion" v-b-tooltip.hover="{ variant: 'success' }" data-toggle="modal" data-target="#resumen-modal" @click="detallesMercancias()">
                                <b v-show="divisa == '1'">
                          ...
                          <div class="row">
                            ...
                            <div class="col-md-10">
                              ...
                              <div class="row mt-2">
                                <div class="col-md-12">
                                  <p style=" border-bottom: 2px solid #00786c; margin-left: -22px; margin-right: -20px;"></p>
                                </div>
                              </div>
                            </div>
                            <div class="col-md-1">
                    ...
                    <div class="row">
                      <b-modal id="resumen-modal" hide-header hide-footer title="INFORMACION GENERAL" title-class="font-18" dialog-class="modal-xl" no-close-on-esc no-close-on-backdrop>
                            ...
                            <div class="col-md-12">
                              <b-container class="bv-example-row" style="background-color: #ffffff">
                                <b-row style=" border-color: #00786c; border-width: 1px; border-bottom-style: solid;">
                                  ...
                                  <b-col cols="3">
                                    <div style=" border: 2px solid #6c757d; width: 150px; align-items: center; padding-right: 20px; padding-left: 20px; margin-right: auto; margin-left: auto; padding-top: 10px; margin-top: auto; padding-bottom: 10px; margin-bottom: auto;">
                                      <b style="font-weight: bold; color: #00786c;">COTIZACIÓN</b>
                                      <br />
                        ...
                        <div class="form-group text-right mt-3">
                          ...
                          <b-button class="width-md ml-1" style="background-color: #00786c" @click="Save2()">Confirmar</b-button>
                        </div>
                      </b-modal>
                    </div>
                  </div>
