...
export default {
  ...
  methods: {
    ...
    generarPDF() {
      ...
      //$('.collapse-horizontal').show(500);

      // Obtener el elemento de la tabla HTML
      const tabla = document.getElementById('tarifario');

      // Convertir la tabla HTML a una imagen utilizando html2canvas
                          ...
                          <div class="table-responsive">
                            <table class="table table-hover" id="tarifario">
                              <thead class="text-white" style="background-color: #2aab5c">
                                <tr>
                                  <th class="col-md text-center">Origen</th>
                                  <th class="col-md text-center">Destino</th>
                                  <th class="col-md text-center">Unidad</th>
                                  <th class="col-md text-center">Costo Kilometraje</th>
                                  <th class="col-md text-center" v-for="(rango, index) in rangos" :key="'rango_titulo' + index">{{ generarTituloColumna(rango) }}</th>
                                </tr>
                              </thead>
                              <tbody style="border: 1px solid #edeff1; border-collapse: collapse;">
                                <template v-for="(origen, origenIndex) in datosOrigenesFTLOcupar" >
                                  ...
                                  <template v-for="(destino, destinoIndex) in datosDestinosFTLOcupar" >
                                    ...
                                    <tr v-for="(dato, dataIndex) in datosUnidaddesFtlOcupar" :key="'tr_' + origenIndex + '_' + destinoIndex + '_' + dataIndex" :class="{ 'fila-color': destinoIndex % 2 === 0 }">
                                      <th class="fixed col-md">{{ origen.estado }} - {{ origen.ciudad }} <br> <span class="badge badge-soft-success" v-if="origen.estatus_geocerca === 1">Comercial</span> <span class="badge badge-soft-warning" v-if="origen.estatus_geocerca === 2">No Comercial</span> <span class="badge badge-soft-danger" v-if="origen.estatus_geocerca === 3">Peligrosa</span></th>
                                      <th class="fixed col-md">{{ destino.estado }} - {{ destino.ciudad }} <br> <span class="badge badge-soft-success" v-if="destino.estatus_geocerca === 1">Comercial</span> <span class="badge badge-soft-warning" v-if="destino.estatus_geocerca === 2">No Comercial</span> <span class="badge badge-soft-danger" v-if="destino.estatus_geocerca === 3">Peligrosa</span></th>
                                      <th class="fixed col-md">{{ dato.name }}</th>
                                      <td class="col-md text-center">${{ formatMoney(dato.precio_kilometraje) }}</td>
                                      <!--td class="col-md" v-for="(rango, rangoIndex) in rangos" :key="'rango_' + rangoIndex">
                                        <button class="btn btn-primary" type="button" v-bind:data-bs-target="'#collapse-' + origen.idGeocerca + destino.idGeocerca + dato.id + rango.max" @click="getInfoCollapse(dato, rango, origen, destino)">${{ formatMoney(generarValorCelda(dato, rango, origen, destino)) }}</button>
                                        <div class="col">
                                          <div class="collapse multi-collapse" v-bind:id="'collapse-' + origen.idGeocerca + destino.idGeocerca + dato.id + rango.max">
                                      ...
                                      <td class="col-md" v-for="(rango, rangoIndex) in rangos" :key="'rango_' + rangoIndex" style="text-align:center;">
...
.fila-color {
    background-color: #f2f2f2; /* Define el color de fondo para las filas pares */
  }
