...
export default {
  ...
  methods: {
    ...
    initMap() {
      const center = { lat: this.lat_centro, lng: this.lng_centro }; // Coordenadas del centro del estado de México
      const radius = 6; // Radio en metros (30 km)
      ...
      function calculatePoint(center, radius, angle) {
        return new google.maps.LatLng(
          center.lat + (radius / 111.32) * Math.sin(angle),
          center.lng + (radius / (111.32 * Math.cos(center.lat))) * Math.cos(angle)
        );
      }

      const polygonPaths = [];
      const numPoints = 500; // Ajusta según la cantidad de puntos que desees en el círculo

      for (let i = 0; i < numPoints; i++) {
        const angle = (i / numPoints) * 2 * Math.PI;
        const point = calculatePoint(center, radius, angle);
      ...
      // Crear un polígono que representa el círculo
      /*const circlePolygon = new google.maps.Polygon({
        paths: polygonPaths,
        fillColor: color, // Color del círculo
        fillOpacity: 0.35,
        strokeWeight: 1,
        map: map,
      });*/
    ...
    /*initMap() {
      ...
      let color = "";
      const radius = 30000;
      const center = { lat: this.all_geo[0][0].lat, lng: this.all_geo[0][0].lng };
    ...
    getCentro() {
      ...
      axios.get('/api/v1/geocercas-centro/', {
        ...
        this.km_red = ((parseFloat(res.data.data[0].kilometros_redonda) / 2) * 1000)
    ...
    eliminaGeocerca() {
      ...
      if(this.cp == ''){
        Swal.fire({
          title: "Codigo Postal",
          text: "Para Eliminar una geocerca, debes seleccionar un codigo postal",
          icon: "error",
          confirmButtonText: "Cerrar",
        });
        return false;
      ...
      const alertaElimina = Swal.mixin({
        customClass: {
          confirmButton: "btn btn-success",
          cancelButton: "btn btn-danger",
        },
      ...
      alertaElimina.fire({
        ...
        text: "Al eliminar la geocerca no se podrá recuperar la geocerca.",
        icon: "warning",
        showCancelButton: true,
        confirmButtonText: "Confirmar",
        cancelButtonText: "Cancelar",
        ...
        if (result.isConfirmed) {
          axios.get('/api/v1/geocercas-eliminar/', {
            params: {
              estado: this.estado,
              cp: this.cp,
              tg: this.tipoGeocerca,
            }
          }).then(res => {
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="row">
              ...
              <div class="col-lg-8">
                <router-link v-if="permisos_agregar == 1" to="/administracion/geocercas-new"
                  class="button btn-dark btn-sm" style="float: left">Nueva Geocerca</router-link>
              </div>
                ...
                <b-tab title="Geocercas Comerciales" active @click="getEstados(1)">
                  <b-alert show variant="light">
                    <b-row>
                      ...
                      <b-col md="2" sm="12">
                        <div>
                          <b-link v-b-toggle.collapse-1><b>¿Geocerca Incorrecta?</b></b-link>
                          <b-collapse id="collapse-1" class="mt-2">
                            <b-button variant="outline-danger" @click="eliminaGeocerca">Eliminar Geocerca</b-button>
                          </b-collapse>
                        </div>
                      </b-col>
                    </b-row>
