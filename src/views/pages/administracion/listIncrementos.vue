...
export default {
  page: {
    title: "Incrementos",
    meta: [{ name: "Incrementos", content: appConfig.description }],
  },
  ...
  methods: {
    ...
    editServicio(item) {
      ...
      this.$router.push(`/edit-incremento/${item.id_incremento}/`);
    },
    ...
    deleteProducto(item) {
      ...
      swalWithBootstrapButtons.fire({
        ...
        text: "¿Deseas eliminar este incremento?",
        icon: 'warning',
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="row">
              ...
              <div class="col-lg-8">
                <router-link v-if="permisos_agregar == 1" to="/administracion/new-incremento" class="button btn-dark btn-sm" style="float: left">Nuevo Incremento</router-link>
              </div>
              ...
              <div class="col-lg-4">
                ...
                <b-input-group v-if="permisos_lectura == 1" v-for="size in ['sm']" :key="size" :size="size" class="mb-2">
                  <b-form-input placeholder="buscar..." v-model="buscador" @keyup.enter="get_incrementos(1)"></b-form-input>
                  <b-input-group-append>
                    <b-button size="sm" text="Button" variant="secondary" @click="get_incrementos(1)"><i class="fa fa-search"></i> Search</b-button>
                  </b-input-group-append>
                </b-input-group>
              </div>
                  ...
                  <b-table v-if="permisos_lectura == 1" class="table table-sm mb-0" :items="all_data" :fields="fields" small bordered responsive="sm" :per-page="perPage" :current-page="currentPage" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :filter="filter" :filter-included-fields="filterOn" @filtered="onFiltered" :selectable="true">
                    <template v-slot:cell(tipo)="data">
                      <div class="text-center">
                        <span class="badge badge-soft-warning" v-if="data.item.tipo === 'i'">Incremento</span>
                        <span class="badge badge-soft-success" v-else>Decremento</span>
                      </div>
                    </template>
                    ...
                    <template v-slot:cell(porcentaje)="data">
                      <div class="text-center">
                        <span >{{ data.item.porcentaje }}%</span>
            ...
            <div class="row">
              <div v-if="permisos_lectura == 1" class="col-lg-12" style="text-align:right">
                ...
                <b-button size="sm" v-show="lastpage" @click="get_incrementos(lastpage)" variant="outline-secondary">Previous page</b-button>
                <b-button size="sm" v-for="index in all" @click="get_incrementos(index)" v-bind:key="index" variant="outline-secondary">{{ index }}</b-button>
                <b-button size="sm" v-show="nextpage" @click="get_incrementos(nextpage)" variant="outline-secondary">Next page</b-button>
              </div>
            </div>
