...
export default {
  ...
  methods: {
    ...
    getCustomer() {
      const pkCustom = this.$route.params.pkCustomer;

      axios
        .get(`/api/v1/customer/${pkCustom}/`)
        .then((response) => {
          ...
          // let cpCo={'codigo_postal':response.data.cp}
          // this.cp.push(cpCo)

          // document.getElementById("form.cp").value = response.data.cp;
    ...
    getDatosComplementarios() {
      const idC = this.$route.params.pkCustomer;
      axios
        .get(`/api/v1/list-datacomplementary/${idC}/`)
        .then((response) => {
