...
export default {
  page: {
    title: "Tipo Envío",
    meta: [{ name: "Tipo Envío", content: appConfig.description }],
  },
  ...
  methods: {
   ...
   editEnvio(item) {
     ...
     this.$router.push(`/edit-envio/${item.id}/`);
   },
      ...
      const swalWithBootstrapButtons = Swal.mixin({
        ...
        swalWithBootstrapButtons.fire({
          ...
          if (result.isConfirmed) {
            ...
            swalWithBootstrapButtons.fire(
              'Eliminado',
              'Tipo Envío Eliminado Exitosamente',
              'success'
            )
          } else if (
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="row">
              ...
              <div class="col-lg-8">                
                <router-link v-if="permisos_agregar == 1" to="/new-envio" class="button btn-dark btn-sm" style="float: left">Nuevo Envío</router-link>
              </div>
                ...
                <div class="table-responsive">
                  ...
                  <b-table v-if="permisos_lectura == 1" class="table table-sm mb-0" :items="all_data" :fields="fields" small bordered responsive="sm" :per-page="perPage" :current-page="currentPage" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :filter="filter" :filter-included-fields="filterOn" @filtered="onFiltered" :selectable="true">
                    <template #cell(actions)="row">
                      ...
                      ||
                      <i v-if="permisos_editar == 1" class="fas fa-edit" @click="editEnvio(row.item)"></i>
                      ||
                      <i v-if="permisos_eliminar == 1" class="fas fa-trash" @click="deleteEnvio(row.item)"></i>
                    </template>                    
                  </b-table>
                </div>
              </div>
