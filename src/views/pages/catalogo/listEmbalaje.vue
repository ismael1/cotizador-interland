...
export default {
  page: {
    title: "Embalaje",
    meta: [{ name: "Embalaje", content: appConfig.description }],
  },
  ...
  methods: {
    ...
    get_embalaje(mypage) {
      ...
      axios.get('/api/v1/list-embalajes-filtro/', {
        params: {
          page: mypage,
          size: this.size,
          palabra: caracter
    ...
    editEmbalaje(item) {
      ...
      this.$router.push(`/edit-embalaje/${item.idEmbalaje}/`);
    },
    ...
    deleteEmbalaje(item) {
      ...
      swalWithBootstrapButtons.fire({
        ...
        text: "¿Deseas eliminar "+item.nombre+" ?",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
          axios({
            method: "put",
            url: `/api/v1/catalogo-deleteEmbalaje/${item.idEmbalaje}/`,
            auth: {
              username: "admin",
              password: "123",
            },
            ...
            Swal.fire({
              title: "Ocurrio un error al intentar eliminar el embalaje. " + error,
              text: "",
              icon: "error",
              confirmButtonText: "Cerrar",
            });
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="row">
              ...
              <div class="col-lg-8">
                <router-link v-if="permisos_agregar == 1" to="/new-embalaje" class="button btn-dark btn-sm" style="float: left">Nuevo Embalaje</router-link>
              </div>
              ...
              <div class="col-lg-4">
                ...
                <b-input-group v-if="permisos_lectura == 1" v-for="size in ['sm']" :key="size" :size="size" class="mb-2">
                  <b-form-input placeholder="buscar..." v-model="buscador" @keyup.enter="get_embalaje(1)"></b-form-input>
                  <b-input-group-append>
                    <b-button size="sm" text="Button" variant="secondary" @click="get_embalaje(1)"><i class="fa fa-search"></i> Search</b-button>
                  </b-input-group-append>
                </b-input-group>
              </div>
              ...
              <div class="col-lg-12">
                <div class="table-responsive">
                  ...
                  <b-table v-if="permisos_lectura == 1" class="table table-sm mb-0" :items="all_data" :fields="fields" small bordered responsive="sm" :per-page="perPage" :current-page="currentPage" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :filter="filter" :filter-included-fields="filterOn" @filtered="onFiltered" :selectable="true">
                    ...
                    <template v-slot:cell(isSubmenu)="data">
                      <div class="text-center">
                        <span class="badge badge-soft-success" v-if="data.item.isSubmenu">Activo</span>
                        <span class="badge badge-soft-danger" v-else>Inactivo</span>
                      </div>
                    </template>
                    <template #cell(actions)="row">
                      <!--i class="fas fa-file-pdf" @click="generatePDF(row.item)"></i-->
                      <i v-if="permisos_editar == 1" class="fas fa-edit" @click="editEmbalaje(row.item)"></i>
                      ||
                      <i v-if="permisos_eliminar == 1" class="fas fa-trash" @click="deleteEmbalaje(row.item)">
                      </i>
                    </template>
                  </b-table>
            ...
            <div class="row">
              <div v-if="permisos_lectura == 1" class="col-lg-12" style="text-align:right">
                ...
                <b-button size="sm" v-show="lastpage" @click="get_embalaje(lastpage)" variant="outline-secondary">Previous page</b-button>
                <b-button size="sm" v-for="index in all" @click="get_embalaje(index)" v-bind:key="index" variant="outline-secondary">{{ index }}</b-button>
                <b-button size="sm" v-show="nextpage" @click="get_embalaje(nextpage)" variant="outline-secondary">Next page</b-button>
              </div>
            </div>
