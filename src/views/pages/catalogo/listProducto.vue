...
export default {
  ...
  methods: {
    ...
    get_customers(mypage) {
      ...
      axios
        .get('/api/v1/list-producto-filtro/', {
          params: {
            page: mypage,
            size: this.size,
            palabra: caracter
   ...
   editProducto(item) {
     ...
     this.$router.push(`/edit-producto/${item.id}/`);
   },
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="row">
              ...
              <div class="col-lg-8">                
                <router-link v-if="permisos_agregar == 1" to="/new-producto" class="button btn-dark btn-sm" style="float: left">Nuevo Producto</router-link>
              </div>
                ...
                <div class="table-responsive">
                  ...
                  <b-table v-if="permisos_lectura == 1" class="table table-sm mb-0" :items="all_data" :fields="fields" small bordered responsive="sm" :per-page="perPage" :current-page="currentPage" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :filter="filter" :filter-included-fields="filterOn" @filtered="onFiltered" :selectable="true">
                    <template #cell(actions)="row">
                      ...
                      ||
                      <i v-if="permisos_editar == 1" class="fas fa-edit" @click="editProducto(row.item)"></i>
                      ||
