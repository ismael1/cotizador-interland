...
export default {
  ...
  methods: {
    ...
    editServicio(item) {
      ...
      this.$router.push(`/edit-servicioCotizacion/${item.id}/`);
    },
      ...
      swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
          axios({
            ...
            Swal.fire({
              title: "Ocurrio un error al intentar eliminar el servicio. " + error,
              text: "",
              icon: "error",
              confirmButtonText: "Cerrar",
            });
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="row">
              ...
              <div class="col-lg-8">
                <router-link v-if="permisos_agregar == 1" to="/new-servicioCotizacion" class="button btn-dark btn-sm" style="float: left">Nuevo Servicio</router-link>
              </div>
                  ...
                  <b-table v-if="permisos_lectura == 1" class="table table-sm mb-0" :items="all_data" :fields="fields" small bordered responsive="sm" :per-page="perPage" :current-page="currentPage" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :filter="filter" :filter-included-fields="filterOn" @filtered="onFiltered" :selectable="true">
                    <template v-slot:cell(status)="data">
                      <div class="text-center">
                        <span class="badge badge-soft-warning" v-if="data.item.status === 0">Inactiva</span>
