...
export default {
  page: {
    ...
    meta: [{ name: "Clave Producto/Servicio", content: appConfig.description }],
  },
  ...
  methods: {
   ...
   editClaveProSer(item) {
     // const idR = this.$route.params.pkRuta;
     this.$router.push(`/edit-clave/${item.id}/`);
   },
   ...
   deleteProducto(item) {
      ...
      const swalWithBootstrapButtons = Swal.mixin({
        ...
        swalWithBootstrapButtons.fire({
          title: 'Eliminar Clave Producto/Servicio',
          ...
          if (result.isConfirmed) {
            ...
            swalWithBootstrapButtons.fire(
              'Eliminado',
              'Clave Producto/Servicio Eliminada Exitosamente',
              'success'
            )
          } else if (
            ...
            swalWithBootstrapButtons.fire(
              'Cancelado',
              'Eliminar Clave Producto/Servicio Cancelado',
              'error'
            )
          }
        })
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="row">
              ...
              <div class="col-lg-8">                
                <router-link v-if="permisos_agregar == 1" to="/new-clave-proser" class="button btn-dark btn-sm" style="float: left">Nueva Clave</router-link>
              </div>
                ...
                <div class="table-responsive">
                  ...
                  <b-table class="table table-sm mb-0" :items="all_data" :fields="fields" small bordered responsive="sm" :per-page="perPage" :current-page="currentPage" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :filter="filter" :filter-included-fields="filterOn" @filtered="onFiltered" :selectable="true">
                    <template #cell(actions)="row">
                      ...
                      ||
                      <i v-if="permisos_editar == 1" class="fas fa-edit" @click="editClaveProSer(row.item)" ></i>
                      ||
