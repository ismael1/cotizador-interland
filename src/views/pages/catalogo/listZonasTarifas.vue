...
export default {
  page: {
    title: "Zonas para Tarifarios",
    meta: [{ name: "Zonas para Tarifarios", content: appConfig.description }],
  },
  ...
  data() {
    return {
      ...
      title: "Lista de Zonas para Tarifarios",
      items: [
        {
          text: "Inicio",
          href: "/",
  ...
  methods: {
    ...
    get_customers(mypage) {
      ...
      axios
        .get('/api/v1/list-zona-tarifa-filtro/', {
          params: {
            page: mypage,
            size: this.size,
            palabra: caracter
        ...
        if (result.isConfirmed) {
          axios({
            ...
            Swal.fire({
              ...
              if (result.isConfirmed) {
                window.location.href = "/catalogo/list-zona-tarifa-filtro";
              }
            })
          }).catch((error) => {
            Swal.fire({
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="row">
              ...
              <div class="col-lg-8">
                <router-link v-if="permisos_agregar == 1" to="/newZonasTarifas" class="button btn-primary btn-sm" style="float: left"><i class="fa fa-plus"></i> Nueva Zona</router-link>
              </div>
              ...
              <div class="col-lg-12">
                <div class="table-responsive">
                  ...
                  <b-table v-if="permisos_lectura == 1" class="table table-sm mb-0" :items="all_data" :fields="fields" small bordered responsive="sm" :per-page="perPage" :current-page="currentPage" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :filter="filter" :filter-included-fields="filterOn" @filtered="onFiltered" :selectable="true">
                    <template v-slot:cell(pais_origen)="data">
                      <div class="text-left">
                        {{ data.item.pais_origen }}, {{ data.item.estado_origen }}, {{ data.item.ciudad_origen }}, C.P. {{ data.item.cp_origen }}
                      </div>
                    </template>
                    <template v-slot:cell(pais_destino)="data">
                      <div class="text-left">
                        {{ data.item.pais_destino }}, {{ data.item.estado_destino }}, {{ data.item.ciudad_destino }}, C.P. {{ data.item.cp_destino }}
                      </div>
                    </template>
                    <template v-slot:cell(clasificacion)="data">
                      <div class="text-center">
                        <span class="badge badge-soft-success" v-if="data.item.clasificacion === 1">Comercial</span>
                        <span class="badge badge-soft-warning" v-else-if="data.item.clasificacion === 2">No Comercial</span>
                        <span class="badge badge-soft-danger" v-else-if="data.item.clasificacion === 3">Peligrosa</span>
                        <span class="badge badge-soft-danger" v-else-if="data.item.clasificacion === 4">Comercial - Peligrosa</span>
                        <span class="badge badge-soft-danger" v-else-if="data.item.clasificacion === 5">No Comercial - Peligrosa</span>
                      </div>
                    </template>
                    <template v-slot:cell(estatus)="data">
                      <div class="text-center">
                        <span class="badge badge-soft-warning" v-if="data.item.estatus === 0">Inactiva</span>
